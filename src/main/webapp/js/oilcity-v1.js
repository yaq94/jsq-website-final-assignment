...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- Initializations ---
    initProspectingSaga(dataStore.prospectingSaga);
    ...
    function initHeroParallax() {
        const heroBg = document.querySelector('#hero-overture .hero-bg');
        if (!heroBg) return;
    ...
    function initProspectingSaga(data) {
        ...
        function updateDetails(index) {
            ...
            detailsText.textContent = item.content;
            
            document.querySelectorAll('.timeline-item').forEach((el, i) => {
                el.classList.toggle('active', i === index);
            });
             timeline.style.transform = `translateX(-${index * 200}px)`;
             currentIndex = index;
        }

        document.querySelectorAll('.timeline-item').forEach((item, index) => {
            item.addEventListener('click', () => updateDetails(index));
        });
        
        prevBtn.addEventListener('click', () => {
            let newIndex = currentIndex > 0 ? currentIndex - 1 : data.length - 1;
            updateDetails(newIndex);
        });
